...
export default function Pergunta({ pergunta, resposta }: PerguntaProps) {
  ...
  return (
    <>
      <div
        className={`bg-[#8fc3e6] rounded-2xl flex flex-col px-3 shadow-md max-w-7xl self-center w-[90%] md:w-[85%] transition-all duration-300 ${aberta ? "pb-3" : ""}`}>
        <div className='flex gap-8 p-3 items-center' onClick={() => setAberta((prev) => !prev)}>
          ...
          <h2 className='font-semibold text-[#0077c8] text-xl sm:text-[1.35rem] md:text-2xl lg:text-[1.75rem] xl:text-[2rem]'>{pergunta}</h2>
        </div>
